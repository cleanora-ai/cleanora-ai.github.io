        ...
        ? [
            {
              ...
              description:
                "Cleanora classifies and moves files with local metadata-based planning and a preview step before changes are applied.",
...
<body>
  ...
  <main class="container">
    ...
    <section class="cta">
      <h2>Organize a folder with Cleanora</h2>
      <p>Classify files locally on Windows, review exact destinations, then apply only the file moves you approve.</p>
      <a class="button" href="${downloadHref}">Download for Windows</a>
    </section>
  </main>
  <footer><div class="container">Cleanora AI - Local, privacy-focused file organization</div></footer>
</body>
</html>`;
}
...
function supportFaqs(page: SupportPage): SeoFaq[] {
  if (page.kind === "feature") {
    return [
      {
        question: `What does ${page.title.toLowerCase()} mean in Cleanora?`,
        answer: page.description,
      },
      {
        question: "Can I review the result before applying changes?",
        answer:
          "Yes. Cleanora is designed to show proposed organization and naming changes for review.",
      },
      {
        question: "Does this workflow require cloud file uploads?",
        answer:
          "No. Cleanora performs its file organization workflow locally.",
      },
    ];
  }
  ...
  return [
    {
      ...
      answer:
        "Use this documentation page as the starting point and contact Cleanora support if the described steps do not resolve the issue.",
    },
    {
      question: "Does Cleanora work locally?",
      answer:
...
<body>
  ...
  <main class="container">
    <section class="answer"><h2>Direct answer</h2><p>${escapeHtml(page.description)} Cleanora keeps the organization workflow local and lets you review changes before applying them.</p></section>
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${escapeHtml(category.title)} | Cleanora</title>
  <meta name="description" content="${escapeHtml(metaDescription)}">
...
- [How Cleanora works](${absoluteUrl("docs/how-it-works")})
...
for (const route of staleRoutes) {
  fs.rmSync(path.join(OUTPUT_DIR, ...route.split("/")), {
    recursive: true,
    force: true,
  });
}
